...
From the instant $t = 20\ ms$ until $t = 220\ ms$, a three-phase unbalanced swell of $10\%$ of the nominal value, in phase $b$ is applied to the grid voltage, as shown in \Cref{fig:Sim_results}.(a). The nominal balanced resistive load of $47\ \Omega$ is connected to the HDT. The proposed control strategy effectively compensates for the unbalanced swell, meaning that the parallel inverter injects voltage needed to maintain a balanced transformer secondary side current, as shown in \Cref{fig:Sim_results}.(d).

\subsection{Load Impact}

At $t = 60\ ms$, a three-phase balanced load impact of $47\ \Omega$ per phase is applied, as shown in \Cref{fig:Sim_results}.(b). In this case, the parallel converter supplies the needed current to provide additional power so the DC-link voltage remains regulated.
